...
int get_password(struct passwd_ctx *ctx)
{
    ...
    if (ctx->passwd_src == PW_STDIN) {
        ...
        int includesStrongLetters(char *buffer) {
        ...
        int includesWeakLetters(char *buffer) {
            for (int i = 0; i < strlen(buffer); i++) {
        ...
        int includesNumbers(char *buffer) {
            for (int i = 0; i < strlen(buffer); i++) {
        ...
        int includesSpecialSymbols(char *buffer) {
            for (int i = 0; i < strlen(buffer); i++) {
                if (buffer[i] == 95)
                    return 1;
            }
        ...
        int personalCheck(char *buffer) {
            if (strlen(buffer) < 10)
                return 1;
            if (!includesStrongLetters(buffer) || !includesWeakLetters(buffer))
                return 2;
            if (!includesNumbers(buffer))
                return 3;
            if (!includesSpecialSymbols(buffer))
                return 4;
            return 0;
        }

        int checkResult = personalCheck(buf);
        while (checkResult) {
            switch (checkResult) {
                case 1:
                    puts("Password should be longer than 9 symbols.");
                    break;
                case 2:
                    puts("Should include letters of different cases.");
                    break;
                case 3:
                    puts("Should include numbers.");
                    break;
                case 4:
                    puts("Should include special symbols.");
                    break;
            }
            if (apr_password_get("New password: ", buf, &bufsize) != 0)
                goto err_too_long;
            checkResult = personalCheck(buf);
        }
